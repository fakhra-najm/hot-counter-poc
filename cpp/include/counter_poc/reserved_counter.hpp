...
public:
    ...
    // Fast-path notification only when a shard becomes exhausted. This lets
    // the engine emit an exact limit-reached event without a global add on
    // every peak-mode transaction.
    bool mark_exhaustion_if_new(std::uint32_t shard) noexcept;
    std::uint32_t exhausted_shards() const noexcept;
...
private:
    struct alignas(64) Shard {
        std::atomic<Amount> used{0};
        Amount reserve{0};
        std::atomic<bool> exhaustion_announced{false};
    };
    Amount limit_;
