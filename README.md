...
\`\`\`text
Client request
    │  Request { counter id, delta, route hash }
    ▼
CounterEngine  ─────────────────────────────── fast data plane
    │
    ├── StrictCounter       one CAS-protected authoritative value
    │
    ├── ReservedCounter     fixed, non-overlapping capacity reservations
    │                         during a safe peak interval
    │
    └── RoutingMap          publishes the active mode after quiescence

HotKeyDetector ── bounded queue ──► control-plane transition request
                                        │
                                        └── coordinator drains admission,
                                            transfers exact state, then
                                            publishes the next routing mode
\`\`\`
